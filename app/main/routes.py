...
def group_function(differentiator, num_groups, students):
	...
	categories = {getattr(student, differentiator.lower()) for student in students} #e.g. {Male, Female} {American, Brazilian, Spanish} {10G, 10W, 9W}

	indices = cycle(''.join(str(x) for x in range(num_groups))) #cycles through the groups e.g. 0 1 2 0 1 2 0 1 2 (if num_groups = 3)

	for category in categories:
		students_with_category = [student for student in students if getattr(student, differentiator.lower()) ==category]
...
def grouper():
	...
	form.differentiator.choices = [("Random", "Random"), ("Gender", "Gender"), ("Homeroom", "Homeroom"), ("Nationality", "Nationality")]
	
	all_classes = Classroom.query.filter_by(teacher_id=current_user.id).all()
	form.classes.choices = [(classroom.name, classroom.name) for classroom in all_classes]

	if form.validate_on_submit():
		students = Classroom.query.filter_by(teacher_id=current_user.id, name=form.classes.data).first().students

		if form.differentiator.data in ["Random", "Gender", "Homeroom", "Nationality"]:
			groups = group_function(form.differentiator.data, form.num_groups.data, students)
